...
QVector<QPair<QString, QString>> ScreenInfoAnalyticsItem::createPayload() const
{
    ...
    payload.append(qMakePair(QStringLiteral("sr"), QStringLiteral("%1x%2").arg(screenSize.width()).arg(screenSize.height())));
    payload.append(qMakePair(QStringLiteral("sd"), QString::number(qApp->primaryScreen()->depth())));
    return payload;
}
